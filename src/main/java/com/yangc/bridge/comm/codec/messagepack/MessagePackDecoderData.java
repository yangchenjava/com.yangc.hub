...
public class MessagePackDecoderData extends ByteToMessageDecoder {
	...
	private boolean decodeResult(int position, ByteBuf in, List<Object> out) throws Exception {
		if (in.readableBytes() >= 4) {
			...
			if (in.readableBytes() >= serializedSize + 2) {
				...
				in.readBytes(data);
				MessagePackResult message = new MessagePack().read(data, MessagePackResult.class);
	...
	private boolean decodeLogin(int position, ByteBuf in, List<Object> out) throws Exception {
		if (in.readableBytes() >= 4) {
			...
			if (in.readableBytes() >= serializedSize + 2) {
				byte[] data = new byte[serializedSize];
				in.readBytes(data);
				MessagePackLogin message = new MessagePack().read(data, MessagePackLogin.class);
	...
	private boolean decodeChat(int position, ByteBuf in, List<Object> out) throws Exception {
		if (in.readableBytes() >= 4) {
			...
			if (in.readableBytes() >= serializedSize + 2) {
				byte[] data = new byte[serializedSize];
				in.readBytes(data);
				MessagePackChat message = new MessagePack().read(data, MessagePackChat.class);
	...
	private boolean decodeReadyFile(int position, ByteBuf in, List<Object> out) throws Exception {
		if (in.readableBytes() >= 4) {
			...
			if (in.readableBytes() >= serializedSize + 2) {
				byte[] data = new byte[serializedSize];
				in.readBytes(data);
				MessagePackFile message = new MessagePack().read(data, MessagePackFile.class);
